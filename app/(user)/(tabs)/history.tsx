...
export default function HistoryScreen() {
  ...
  const generateFullReport = async () => {
    ...
    try {
      const tableRows = safeHistoryData.map(item => `
        <tr>
  ...
  // 4. Real-time Summary
  const totalScans = safeHistoryData.length.toString();
  const highRiskCount = safeHistoryData.filter(item => item?.risk === 'High').length.toString();
  
  const getLatestMonth = () => {
    if (safeHistoryData.length === 0 || !safeHistoryData[0]?.timestamp) return '-';
    const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    const dateObj = new Date(safeHistoryData[0].timestamp);
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <Text style={{ textAlign: 'center', color: '#94A3B8', marginTop: 30, fontSize: 16 }}>
            No history records found. Or check your connection.
          </Text>
        ) : (
          safeHistoryData.map((record, index) => {
            const key = record?.id ? record.id.toString() : `fallback-key-${index}`;
            const riskStyle = getRiskStyle(record?.risk || 'Unknown');
            
            return (
              <View key={key} style={styles.recordCard}>
                <View style={styles.recordHeader}>
                  <View style={styles.titleArea}>
                    <Text style={styles.recordTitle}>{record?.condition || 'Unknown Condition'}</Text>
                    <View style={[styles.riskBadge, { backgroundColor: riskStyle.bg }]}>
                      <Text style={[styles.riskText, { color: riskStyle.text }]}>{record?.risk || 'Unknown'}</Text>
                    </View>
                  </View>
                ...
                <View style={styles.recordDetails}>
                  <Text style={styles.detailText}>Confidence: <Text style={styles.confidenceValue}>{record?.confidence || '0%'}</Text></Text>
                  <Text style={styles.detailText}>{record?.date || 'Unknown Date'}</Text>
                </View>
              </View>
            );
          })
